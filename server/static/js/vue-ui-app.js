...
const createUIApp = function(mountSelector='#mini_app', connectionsData) {
    /*
    ...
    let app = {
        ...
        , getGraphNodeElement(name){

            /* return the real node for the given name.*/
            return document.getElementById(`node_${name}`).parentNode
        }
        ...
        , spawnWindow(conf={name: this.newPanelName}) {
            ...
            let item = nodesApp.spawnRandom(conf.name)
            setTimeout(e=>{
                let allowPan = true
                enableDragging(this.getGraphNodeElement(item.name), allowPan)
            }, 50)
            this.newPanelName =  Math.random().toString(32).slice(3, 7)
            return r
        }
...
const createNodesApp = function(mountSelector='#panspace_container', connectionsData) {
    /*
        This Vue app maintains the ui clickable buttons
        and the windows made by winbox.
    ...
    let item = function(inCount, outCount, name=undefined){
        name = name ? name: Math.random().toString(32).slice(3, 7)

        let pipsInbound = []
            , pipsOutbound = []

        Array(inCount).fill(0).forEach((e,i)=>{
            pipsInbound.push({
                label: name
                , index: i
            })
        ...
        Array(outCount).fill(0).forEach((e,i)=>{
            pipsOutbound.push({
                label: name
                , index: i
            })
    ...
    const initBoxes = []
    for (var i = 0; i < connectionsData.nodes.length; i++) {
        let conn = connectionsData.nodes[i]
        let _node = item(1,1, conn.name)

        initBoxes.push(_node)
    }

    for (var i = 0; i < connectionsData.connections.length; i++) {
        let conn = connectionsData.connections[i]
    ...
    let app = {
        ...
        , panelFocus(e, item){
            console.log('panelFocus', item, e)
            dispatchFocusNodeEvent(item)

            let n = this.getGraphNodeElement(item.name)
            n.parentElement.querySelectorAll('.focus')
